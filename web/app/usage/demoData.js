...
export function buildDemoPayload(days = 30) {
  const now = new Date();
  const daily = [];
  const totals = {
  ...
  const mkModel = (model, label, f, hitRate) => {
    const t = scaleDay(f * 8);
    const context = t.inputTokens + t.cacheCreationTokens + t.cacheReadTokens;
    return {
      model, label, approximatePricing: false, source: "claude-code",
